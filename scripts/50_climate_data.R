...
rs_base <- list.files(paste0(getwd(), "/output/Sen2/indices/"), pattern = "*.tif")[1]
winddir <- "data/data_D_MN003.csv" #Winddirection data 
windspeed <- "/wind_speed/data/data_F_MN003.csv"#DWD Climate Data Center (CDC): Stündliche Stationsmessungen der Windrichtung in 10 m Höhe in Grad für Deutschland, Version v19.3, abgerufen am <Datum>.

#set Variables
used_climate <- c("air_temp_mean", "drought", "evapo", "precipitation", "sunshine", "soil_moist")
...
climate_preproc<- pblapply(climate_crop_list, function(x){
  ...
  # preprocessing made according to DWD Climate Data Center (CDC), Raster der Monatsmittel der Lufttemperatur (2m) für Deutschland, Version v1.0.
  if(grepl("air_temp_mean", names(x)) == TRUE){
    x <- x / 10
  }
  
  # preprocessing made according to DWD Climate Data Center (CDC), Raster des monatlichen Trockenheitsindex nach de Martonne für Deutschland, Version v1.0.
  else if(grepl("drought", names(x)) ==TRUE){
    values(x)[values(x) == -999] <- NA
    x
  }
  # preprocessing made accourding to DWD Climate Data Center (CDC): Monatliche Raster der realen Evapotranspiration über Gras und sandigem Lehm, Version 0.x, aktuelles Datum.
  else if(grepl("evapo", names(x)) == TRUE){
    x <- x / 10
  }
  
  # preprocessing made accourding to DWD Climate Data Center (CDC), Raster der Monatssumme der Niederschlagshöhe für Deutschland, Version v1.0.
  else if(grepl("precipitation", names(x)) == TRUE){
    values(x)[values(x) == -999] <- NA
    x
  }
  
  # preprocessing made accourding to DWD Climate Data Center (CDC): Monatliche Raster der mittleren Bodenfeuchte unter Gras und sandigem Lehm, Version 0.x, aktuelles Datum.
  else if(grepl("soil_moist", names(x)) == TRUE){
    x <- x / 100
  }
  
  #preprocessing made accourding to DWD Climate Data Center (CDC), Raster der Monatssumme der Sonnenscheindauer für Deutschland Version v1.0.
...
pb = txtProgressBar(min = 0, max = nlayers(climate_preproc), initial = 0, style = 3)

#resample to 10 m; method biliniar because climate is not stationairy
for (k in 1:nlayers(climate_preproc)){
  print(paste(Sys.time(), "Resampling raster", k, "of", nlayers(climate_preproc),"..."))
  if (k == 1){
    l <-list()
    l[[1]] <- gdal_resample(climate_preproc[[k]],rs_base[[1]], method = "bilinear")
    names(l[[1]]) <- names(climate_preproc)[[1]]
  }
  else{
    l[[k]] <- gdal_resample(climate_preproc[[k]],rs_base[[1]], method = "bilinear")
...
#writing raster out
terra::writeRaster(climate_resample, paste0(getwd(), "/output/Parameter/climate.tif"))
