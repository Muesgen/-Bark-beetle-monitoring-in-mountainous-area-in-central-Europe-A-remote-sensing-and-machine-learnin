...
# Description: This Script calculate vegetation indices on Sentinel 2
#images and crop then to exten of interest
# its logic and structure is adapted from Msoffe F & Zeuss D
#(under review) Automatic Processing of Sentinel-2 Data for Monitoring
#Biodiversity in a User-Defined Area: An example from Mount Kilimanjaro
#National Park
#---

#Set paths
setwd("D:/Marvin/BB_rf") # working directory
pckgs <- c("raster", "rgdal","rgeos","stringr","terra","sp","gdalUtils")
# needed packages
funfold <-  paste0(getwd(),"/functions") #path to function folder
area <- paste0(getwd(),"/input/observations/NP_Boundary.shp")
# path to area of interes
Input_dir <- paste0(getwd(),"/input/Sentinel_2_l2a/")
# path to the input directory where senitnel 2 l2a images are located

# Set Variables
sentinel_tile <- "32UMU" # tiles of interes
pckgs <- c("raster", "rgdal","rgeos","stringr","terra","sp","gdalUtils")
# needed packages
vegetation_indices <- c("NDRE", "DSWI4", "MSI", "CIrededge","MSR670",
                        "NGRDI", "MVI", "ExGR", "GLI", "MCARI", "GNDVI",
                        "NDVI") # Indices which you want to calculate
...
paths_sentinel_tiles <- list.files(path_sentinel_data,
                        pattern = sentinel_tile, full.names = TRUE,
                        recursive = FALSE)
...
rlist<- lapply(paths_sentinel_tiles, function(x){
  ...
  path_band_10m <- list.files(file.path(x, "GRANULE"),
                        pattern = searchPattern_10m, full.names = TRUE,
                        recursive = TRUE)[1]
})

# create progressbar
pb = txtProgressBar(min = 0, max = length(paths_sentinel_tiles), initial = 0,
                    style = 3)

#starting processing
for(i in 1:length(paths_sentinel_tiles)){ # iterate over all sentinel
  #scene folders in the sentinel data folder
  ...
  paths_bands_10m <- list.files(file.path(paths_sentinel_tiles[i],
                      "GRANULE"), pattern = searchPattern_10m,
                      full.names = TRUE, recursive = TRUE)
  paths_bands_20m <- list.files(file.path(paths_sentinel_tiles[i],
                      "GRANULE"), pattern = searchPattern_20m,
                      full.names = TRUE, recursive = TRUE)
  ...
  for (j in 1:nlayers(bands_20m)){
    print(paste(Sys.time(), "Resampling raster", j, "of",
                nlayers(bands_20m),"..."))
    if (j == 1){
      l <-list()
    ...
    # if an error occurs:  Error in rgdal::putRasterData(x@file@transient,
    #v, band = 1, offset = off) :
  ...
  print("bands stacked")
  ##ATTENTION:
  #possible Error when a gdal version without the plugin (Jpeg2000openJPeg)
  is not installed.
  ...
  #checking if Values are in the area of itnerest, if not the loop jump to
  #start
  if (bands_mask@data@min == 0 && bands_mask@data@max == 0){
    print(paste0("Sentinel Image: ",filenames[i]," does not have any Values
                 in studyarea. Loop continues with next image"))
    next
  }
  print("bands masked")
  ...
  RS_indices <- calc_Indices(bands_crop, rgbi = veg_indices) #new indices
  #calc function, input to function must be changed
  print("indices calculated")

  # Write Indices as Images to disk
  jpeg(filename = paste0(getwd(), "/output/graphics/indices","/",
                         str_sub(filenames[i],end = -6), ".jpg"),
  ...
  #create output file
  fname <- paste0(getwd(),"/output/Sen2/indices/",str_sub(filenames[i],
                                                end = -6),"_indices.tif")
...
######## End indices calculation and cropping ##############
