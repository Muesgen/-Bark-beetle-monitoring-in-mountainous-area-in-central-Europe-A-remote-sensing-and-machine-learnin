# -*- coding: utf-8 -*-
########## Sentinele 1 Preprocessing ##########
#---
# title: "Snappy Preprocessing"
# Author: Marvin Müsgen
# Description: Preprocess Sentinel 1 Images via the snap
#python interface snappy
# The scripts sources functions from the function folder.
# Preproccessing steps are: Thermal noise removal, apply orbit file,
# radiometric calibration,
# terrain correction, subsetting, converting to db and calculate
#polarization ratio
...
from PreProcSen_1 import (do_speckle_filtering, do_calibration,
do_apply_orbit_file, do_thermal_noise_removal,do_terrain_correction,do_subset,
do_convert_linear_to_db, do_ratio_VH_VV, do_merge_products)
...
path = "D:\Marvin\BB_rf\input\sentinel_1_data\Descending"
# Supperfolder where the sentinel1 image folders are in
outputpath = "D:\Marvin\BB_rf\input\sentinel_1_preproc\Descending"
...
#PARAMETER["central_meridian", -159.0],PARAMETER["latitude_of_origin", 0.0],
...
for folder in os.listdir(path):
   ...
   ###Terrain Correction
   ## IW images can be downsampled from 10m to 40m
   #(the same resolution as EW images). by changing downsample = 1
   if (modestamp == 'IW' and productstamp == 'GRDH') or
   (modestamp == 'EW' and productstamp == 'GRDH'):
       down_tercorrected = do_terrain_correction(sentinel_1proc, proj, 0,
       polarization)
       ...
       print("Writng...")
       ProductIO.writeProduct(Bandmerge, outputpath + '\\' + folder[:-5] + '_10',
       'GeoTIFF')
