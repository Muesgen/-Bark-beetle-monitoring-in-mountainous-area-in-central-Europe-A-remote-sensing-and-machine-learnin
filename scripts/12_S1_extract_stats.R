############# SKript for extracting Data from Rasters
#                       and calculate statistics#############

#---
# title: "Extractin Raster"
# Author: Marvin Müsgen
# Description: Allign Raster with different extent.
# Then Data will be extracted.
# After Data extraction 4 tatistics are calculated
# for the extracted data, per month and season.
# The statistitcs are min, max, mean, sd.
# At the End this statistics will be also calculated
#for each month and season of the raster input stack
# This works for Scenario 1 and also after adaption for
#Scenario 2
#---


######Setting parameters#########
observations <- "barkbeetle_obs_x_to_2018_2class.csv"
# name of observation data
...
pckgs <- c("velox","raster","rgdal","pbapply","doParallel",
           "gdalUtils","rgeos","sp", "matrixStats","dplyr","terra")
...
xcoordcolnum <- 3 # column number of the x coordinate
ycoordcolnum <- 4 # column number of the y coordinate
projObs <- "+proj=longlat +datum=WGS84 +no_defs "
...
files_dates<- as.character(as.Date(substr(list.files(paste0(getwd(),
                            Input_dir, Sat_dir, sub_dir,"/"),
                            full.names = FALSE, pattern = "ext.tif$"),
                            18, 26), format="%Y%m%d"))
...
print(month)
rsstack <- raster::stack(paste0(getwd(),Input_dir,Sat_dir, sub_dir,
...
#reading the rasters
temp <- list.files(paste0(getwd(), Input_dir, Sat_dir, sub_dir,"/"),
...
if (lapply(ras_list, function(X){ extRas@extent == X@extent }) == TRUE){
  ...
  r_list<- pbsapply(temp, function(x){
    gdalUtils::align_rasters(x,temp[[1]],
                             dstfile = sub('\\.tif', '_ext.tif', x),nThreads = 7)
  })
  #stackig rasters
  temp2 <- list.files(paste0(getwd(),
                             Input_dir, Sat_dir, sub_dir,"/"),
                              pattern = "*_ext.tif", full.names = TRUE)
  rs <- stack(temp2)
}else{
...
obs <- read.csv("E:/Marvin/BB_rf/input/observations/
                new/barkbeetle_obs_x_to_2018_2class.csv")
...
temp2 <- list.files(paste0(getwd(), Input_dir, Sat_dir, sub_dir,"/"),
                    pattern = "*ext.tif", full.names = TRUE)
temp
rs <- stack(temp2)
#r_crs<- rs[[1]]@crs
...
if(length(rs@layers) == length(files_dates)){
  ...
  print("Rasterstack with single Layer was detected. Rename colnames in
        files_dates")
} else {
  #files_d<- lapply(files_dates, function(x){
  #  replicate(length(rs@layers)/length(files_dates), x)
  # replicate names for renaming colnames
  #  })
  #print(paste0("Rasterstack with ", length(rs@layers)/length(files_dates),
  #" layer per raster was detected. Renaming colnames
  #for multibandraster was succesful"))
  #files_d<- unlist(files_d, recursive = FALSE) # unlisting nested lists
  n <- length(rs@layers) / length(files_dates)
  for (i in 1: length(files_dates)){#renaming colnames with dates and
    polarization
...
write.csv(obs,file = paste0("E:/Marvin/BB_rf/output/new/
                            Sen_1_2/Ascending/
                            _Extracted_data_2",start,end,".csv"))


terra::writeRaster(rs, paste0("E:/Marvin/BB_rf/
                              output/new/Sen_1_2/Ascending
                              /_Sen1_Ascending_Stack2.tif"), overwrite=TRUE)
...
pblapply(pols,function(x){
  ...
  # standard deviaton
  write.csv(obs_stats, file = paste0("E:/Marvin/BB_rf/output/new/
            Sen_1_2/Ascending/observation_stats",start,end,x,"2.csv"))
  lapply(month, function(y){
    ...
    # standard deviaton
    write.csv(month_stats, file = paste0("E:/Marvin/BB_rf/output/
                  new/Sen_1_2/Ascending/","observation_stats",x,y,"2.csv"))
    #write.csv(month_stats, file = paste0("E:/Marvin/BB_rf/
    #output/Sen1_3/observation_stats",x,y,"3.csv"))
...
pblapply(pols, function(x){
  print(paste0("start calulating raster statistics for whole
               time period of polarization: ",x," ",Sys.time()))
  rs_stat <- rs[[which(grepl(x,name) == TRUE)]]
  print("mean...")
  beginCluster()
  rs_mean <- na.exclude(stackApply(rs_stat,
                                   indices= rep(1,nlayers(rs_stat)), fun = mean))
  print("min...")
  rs_min <- stackApply(rs_stat,
                       indices= rep(1,nlayers(rs_stat)), fun = min, na.rm = TRUE)
  print("max...")
  rs_max <- stackApply(rs_stat,
                       indices= rep(1,nlayers(rs_stat)), fun = max, na.rm = TRUE)
  rs_sd <- clusterR(rs_stat,fqsd)

  endCluster()
  print(paste0(" raster statistics are calculated
               succesfully for whole time period of polarization: ",x," "
               ,Sys.time()))
  print(paste0("start writing raster statistics f
               or whole time period: ",x," ",Sys.time()))
  terra::writeRaster(rs_mean,
                     paste0("E:/Marvin/BB_rf/output/new/Sen_1_2/Ascending/"
                    ,x,"_mean_from",start,"_to_",end,"2.tif"), overwrite=TRUE)
  terra::writeRaster(rs_max,
                     paste0("E:/Marvin/BB_rf/output/new/Sen_1_2/Ascending/"
                    ,x,"_max_from",start,"_to_",end,"2.tif"), overwrite=TRUE)
  terra::writeRaster(rs_min,
                     paste0("E:/Marvin/BB_rf/output/new/Sen_1_2/Ascending/"
                    ,x,"_min_from",start,"_to_",end,"2.tif"), overwrite=TRUE)
  terra::writeRaster(rs_sd,
                     paste0("E:/Marvin/BB_rf/output/new/Sen_1_2/Ascending/"
                    ,x,"_sd_from",start,"_to_",end,"2.tif"), overwrite=TRUE)
  rm(rs_mean,rs_max,rs_min,rs_sd)
  print(paste0("Finish writing raster statistics for whole time period: ",x," "
               ,Sys.time()))
  pblapply(month, function(y){
    m_dates<- as.character(as.Date(substr(names(rs_stat), 18, 26),
                                   format="%Y%m%d"))
    ...
    print(paste0("start calulating raster statistics for month ",
                 y," of polarization: ",x," ",Sys.time()))
    #rs_stat <- rs_month[[which(grepl(x,names(rs_month)) == TRUE)]]
    beginCluster()
    print("mean...")
    rsm_mean <- na.exclude(stackApply(rs_month,
                                      indices= rep(1,nlayers(rs_month)),
                                      fun = mean))
    print("min...")
    rsm_min <- stackApply(rs_month,
                          indices= rep(1,nlayers(rs_month)), fun = min,
                          na.rm = TRUE)
    print("max...")
    rsm_max <- stackApply(rs_month,
                          indices= rep(1,nlayers(rs_month)), fun = max,
                          na.rm = TRUE)
    rsm_sd <- clusterR(rs_month,fqsd)
    print("median...")
    endCluster()
    print(paste0("Finished calculating raster
                 statistics for month ",y," of polarization: ",x," ",
                 Sys.time()))
    print(paste0("start writing raster
                 statistic for whole time period: ", x, " ",Sys.time()))
    terra::writeRaster(rsm_mean, paste0("E:/Marvin/BB_rf/
    output/new/Sen_1_2/Ascending/",x,"_mean_from ",y,"2.tif"), overwrite=TRUE)
    terra::writeRaster(rsm_max, paste0("E:/Marvin/BB_rf/
    output/new/Sen_1_2/Ascending/",x,"_max_from ",y,"2.tif"), overwrite=TRUE)
    terra::writeRaster(rsm_min, paste0("E:/Marvin/BB_rf/
    output/new/Sen_1_2/Ascending/",x,"_min_from ",y,"2.tif"), overwrite=TRUE)
    terra::writeRaster(rsm_sd, paste0("E:/Marvin/BB_rf/
    output/new/Sen_1_2/Ascending/",x,"_sd_from ",y,"2.tif"), overwrite=TRUE)
    rm(rs_month,rsm_mean,rsm_max,rsm_min,rsm_sd)
    print(paste0(" raster statistics for month ",
                 y,"of polarization ",x," are written succesfully"))
  })
  rm(rs_stat)
})

########### END Data extraction and statistic calculation
