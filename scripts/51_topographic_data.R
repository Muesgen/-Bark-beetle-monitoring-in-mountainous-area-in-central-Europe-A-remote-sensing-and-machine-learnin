########## Creation of useable topographic Data##########
#---
# title: "Prepare topographic Data"
# Author: Marvin Müsgen
# Description: In this Skript the topographic Input data will be preprocessed
#---

#Setting Paths
dem <- "eu_dem_v11_E40N20.TIF" # name of observation
# DOwnloaded from https://land.copernicus.eu/imagery-in-situ/eu-dem/
#eu-dem-v1.1/fetch-land-file?hash=97824c12f357f50638d665b5a58707cd82857d57
...
rs_base <- "/output/Sen2/Indices/
S2A_MSIL2A_20180405T103021_N0207_R108_T32UMU_20180405T141122_indices.tif"
#path to extent base
winddir_file <- "/input/observations/climate_data/data/data_DD_MN008.csv"
#Winddirection data
windspeed_file <- "/input/observations/climate_data/wind_speed/data/
data_F_MN003.csv"#windspeed dataDWD Climate Data Center (CDC):
#St?ndliche Stationsmessungen der Windrichtung in 10 m H?he in Grad f?r
#Deutschland, Version v19.3, abgerufen am <Datum>.
...
rsaga.slope(in.dem = dem_file, out.slope = paste0(getwd(), Input_dir,
                                                  "/slope.sgrd"), env = env)
...
for(i in 1:length(month)){
  ...
  if(i == length(month)){
    ...
    #calculate for month
    winddir_temp <- winddir[which(grepl(month[i],
                                        winddir$Zeitstempel) == TRUE),]
    WD <- winddir_temp$Wert
    WS <- Windspeed$Wert[which((Windspeed$Zeitstempel %in%
                                  winddir_temp$Zeitstempel) ==TRUE)]
...
rsaga.get.modules(libs = "ta_morphometry", env = env)
rsaga.get.usage(lib = "ta_morphometry", env = env,
                module = "Wind Effect (Windward / Leeward Index)")

#initialize progressbar
pb = txtProgressBar(min = 0,
                    max = nrow(weighted_winddirection), initial = 0, style = 3)
...
for(i in 1:nrow(weighted_winddirection)){
  ...
  #creating outputfile
  wind_effect_file <- paste0(getwd(),
              Input_dir,"Wind_effect_",weighted_winddirection$date[i],".sgrd")
...
rsaga.get.usage(lib = "ta_lighting", env = env,
                module = "Potential Incoming Solar Radiation")
rsaga.get.usage(lib = "ta_lighting", env = env,
...
############END Creation of useable topographic Data #################
