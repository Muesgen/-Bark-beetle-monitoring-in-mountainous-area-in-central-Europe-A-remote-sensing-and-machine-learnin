...
windspeed <- "/wind_speed/data/data_F_MN003.csv"#DWD Climate Data Center (CDC): St?ndliche Stationsmessungen der Windrichtung in 10 m H?he in Grad f?r Deutschland, Version v19.3, abgerufen am <Datum>.
...
climate_preproc<- pblapply(climate_crop_list, function(x){
  ...
  # preprocessing made according to DWD Climate Data Center (CDC), Raster der Monatsmittel der Lufttemperatur (2m) f?r Deutschland, Version v1.0.
  if(grepl("air_temp_mean", names(x)) == TRUE){
    x <- x / 10
  }
  
  # preprocessing made according to DWD Climate Data Center (CDC), Raster des monatlichen Trockenheitsindex nach de Martonne f?r Deutschland, Version v1.0.
  else if(grepl("drought", names(x)) ==TRUE){
    values(x)[values(x) == -999] <- NA
    x
  }
  # preprocessing made accourding to DWD Climate Data Center (CDC): Monatliche Raster der realen Evapotranspiration ?ber Gras und sandigem Lehm, Version 0.x, aktuelles Datum.
  else if(grepl("evapo", names(x)) == TRUE){
    x <- x / 10
  }
  
  # preprocessing made accourding to DWD Climate Data Center (CDC), Raster der Monatssumme der Niederschlagsh?he f?r Deutschland, Version v1.0.
  ...
  #preprocessing made accourding to DWD Climate Data Center (CDC), Raster der Monatssumme der Sonnenscheindauer f?r Deutschland Version v1.0.
