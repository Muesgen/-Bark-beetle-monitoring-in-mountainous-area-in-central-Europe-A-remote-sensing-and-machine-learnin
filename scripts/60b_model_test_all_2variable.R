...
files_2<- list.files(paste0(getwd(),"/modelinput1/Sen2/"), pattern = "observation")
...
if(length(files_2) == 1){
  data2 <- read.csv(paste0(getwd(),"/modelinput1/Sen2/",files_2))
}else{
  for (i in 1:length(files_2)){
    if (i == 1){
      df <- read.csv(paste0(getwd(),"/modelinput1/Sen2/",files_2[i]))
      df <- df[,c(2,3,4,5)]
    }
    else if(i == length(files_2)){
      temp <- read.csv(paste0(getwd(),"/modelinput1/Sen2/",files_2[i]))
      temp <- temp[,c(2,3,4,5)]
      df <- cbind(df,temp)
...
files_1a <- list.files(paste0(getwd(),"/modelinput1/Ascending/"), pattern = "observation_stats")
files_1d <- list.files(paste0(getwd(),"/modelinput1/Descending/"), pattern = "observation_stats")

### reading Sentinel 1 Data ###
#this elaborate process prevents errors in the training data
if(length(files_1a) == 1 & length(files_1d) ==1){
  dfA <- read.csv(paste0(getwd(),"/modelinput1/Ascending/",files_1a))
  dfD <- read.csv(paste0(getwd(),"/modelinput1/Descending/",files_1d))
  ...
  for (x in 1:length(files_1a)){
    if (x == 1){
      dfA <- read.csv(paste0(getwd(),"/modelinput1/Ascending/",files_1a[x]))
      dfD <- read.csv(paste0(getwd(),"/modelinput1/Descending/",files_1d[x]))
      colnames(dfA) <- paste0(colnames(dfA),"_ASCEND")
      colnames(dfD) <- paste0(colnames(dfD),"_DESCEND")
      df1 <- cbind(dfA, dfD)
    }
...
data3 <- read.csv(paste0(getwd(),"/modelinput1/paramet_data2.csv"), sep = ",")
...
obs <- read.csv2(paste0(getwd(),"/input/observations/Befall_gesamt/barkbeetle_obs_x_to_2018_2class.csv"), sep=",")
...
dummy <- rgdal::readOGR("E:/Marvin/NP_Boundary.shp")
...
spdftrans<- sp::spTransform(spdf,raster::crs(north))
